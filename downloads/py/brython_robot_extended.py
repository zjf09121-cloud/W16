...
class World:
    def __init__(self, width, height):
        self.width = width
        self.height = height
        self.occupied = set()  # 追蹤所有 robot 佔據位置
        ...
        def action(next_done):
            def step():
                ...
                if not (0 <= next_x < self.world.width and 0 <= next_y < self.world.height):
                    print(f"Robot #{self.id} 撞到牆壁！位置 ({next_x+1}, {next_y+1})")
                    next_done()
                    return

                if (next_x, next_y) in self.world.occupied:
                    print(f"Robot #{self.id} 撞到另一個 robot！位置 ({next_x+1}, {next_y+1})")
                    next_done()
                    return
